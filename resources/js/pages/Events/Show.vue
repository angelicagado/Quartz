  ...
  >
    ...
    <section v-if="event.sessions?.length" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-serif text-lg font-bold text-slate-900 dark:text-slate-100">Event Sessions</h3>
            <span v-if="event.attendance_type" class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-800 dark:bg-slate-800 dark:text-slate-200 capitalize">
                Attendance Type: {{ event.attendance_type.replace('-', ' ') }}
            </span>
        </div>
