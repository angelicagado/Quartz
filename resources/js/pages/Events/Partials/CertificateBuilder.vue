<template>
    ...
    >
        <h3
            class="font-serif text-lg font-bold text-slate-900 dark:text-slate-100"
        >
            Certificate Builder
        </h3>
        <form @submit.prevent="submit" class="mt-4 space-y-4">
            ...
            <div
                v-if="previewUrl"
                class="relative mt-4 border border-slate-200 dark:border-slate-700"
                style="overflow: auto"
            >
                <p class="mb-2 p-2 text-xs text-slate-500">
                    Click on the image to set the position for the selected
                    field.
                </p>
                ...
                <div
                    class="relative inline-block cursor-crosshair overflow-hidden border border-slate-300"
                    @click="setImageCoordinates"
                >
                    <img
                        :src="previewUrl"
                    ...
                    >
                        ...
                        <input
                            type="number"
                            v-model="field.size"
                            class="ml-2 w-16 p-0 text-xs text-black"
                            @click.stop
                            title="Font Size"
                        />
                    </div>
            ...
            <div class="flex justify-end pt-4">
                <button
                    type="submit"
                    class="rounded-xl bg-slate-900 px-6 py-2.5 font-bold text-white shadow hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900"
...
const form = useForm({
    ...
    dynamic_fields_mapping:
        props.existingTemplate?.dynamic_fields_mapping || {},
});
