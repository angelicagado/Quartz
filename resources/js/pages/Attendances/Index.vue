<template>
    ...
    <div
        class="mx-auto max-w-5xl animate-in space-y-8 pb-20 duration-500 fade-in"
    >
        <div
        ...
        <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-12">
            <div class="lg:col-span-12">
                <div
                    class="relative overflow-hidden rounded-[2.5rem] border border-slate-800 bg-slate-900 p-6 shadow-2xl sm:p-10 dark:border-slate-700 dark:bg-slate-800"
                >
                    <div
...
const handleScan = async (token: string, type: string) => {
    ...
    try {
        ...
        if (response.data.status === 'success') {
            const newScan = {
                ...
                time:
                    ...
                    new Date(response.data.data.scanned_at).toLocaleString(
                        'en-US',
                        {
                            month: 'short',
                            day: 'numeric',
        ...
        const message =
            error.response?.data?.message || 'Invalid Scan / Connection Error';
