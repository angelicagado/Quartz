<template>
  <Head title="System Users" />
    <div class="mx-auto w-full max-w-7xl pb-10 font-['Outfit']">
      ...
      <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="overflow-x-auto">
          <table class="w-full border-collapse text-left">
            <thead>
              <tr class="border-b border-slate-200 bg-slate-50 text-sm font-semibold tracking-wide text-slate-500 uppercase dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-400">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr v-for="user in filteredUsers" :key="user.id" class="group transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-100 font-bold text-slate-500 uppercase dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
                      {{ user.name.charAt(0) }}
                    </div>
                    <div>
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center gap-1.5">
                    <Shield
                      class="h-4 w-4"
                      :class="user.role.name === 'super_admin' ? 'text-rose-500' : user.role.name === 'admin' ? 'text-[#C5A059]' : user.role.name === 'organizer' ? 'text-indigo-500' : 'text-slate-400'"
                    />
                ...
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      :class="user.status === 'active' ? 'text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10' : 'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10'"
                    >
      ...
      <div v-if="isCreateModalOpen || isEditModalOpen" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4 pb-20 sm:p-6">
        <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" @click="closeModal"></div>
        <div class="relative my-auto flex w-full max-w-lg animate-in flex-col rounded-2xl border border-slate-100 bg-white shadow-2xl duration-200 zoom-in-95 fade-in dark:border-slate-800 dark:bg-slate-900">
          <!-- Modal Header -->
            ...
            <form @submit.prevent="submit" class="space-y-5">
              ...
              <div class="grid grid-cols-2 gap-4 pt-1">
                <div>
                  ...
                  <p v-if="form.errors.role_id" class="mt-1 text-sm text-rose-500">{{ form.errors.role_id }}</p>
                </div>
                <div>
                  <label class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-300">
                    Account Status *
              ...
              <!-- Actions -->
              <div class="mt-6 flex items-center justify-end gap-3 border-t border-slate-100 pt-6 pb-2 dark:border-slate-800">
                <button
                  type="button"
                  @click="closeModal"
                ...
                >
                  {{ form.processing ? 'Saving...' : editingUser ? 'Update Record' : 'Create Record' }}
                </button>
              </div>
            </form>
          </div>
