          ...
          <table class="w-full text-left whitespace-nowrap">
            <thead>
              <tr class="border-b border-slate-100 dark:border-slate-800">
                <th class="px-8 py-5 text-sm font-bold tracking-wider text-slate-900 uppercase dark:text-slate-300">
                  Participant
                </th>
                <th class="px-8 py-5 text-sm font-bold tracking-wider text-slate-900 uppercase dark:text-slate-300">
                  Event
                ...
                <th class="px-8 py-5 text-right text-sm font-bold tracking-wider text-slate-900 uppercase dark:text-slate-300">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-50 dark:divide-slate-800/50">
              <tr v-if="filteredParticipants.length === 0">
                <td colspan="4" class="px-8 py-12 text-center font-light text-slate-400 italic dark:text-slate-500">
              ...
              >
                <td class="px-8 py-6">
                  <div class="flex items-center gap-4">
                    <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 text-slate-400 transition-all group-hover:bg-[#C5A059]/10 group-hover:text-[#C5A059] dark:bg-slate-800 dark:text-slate-500">
                      <Users class="h-6 w-6" />
                    </div>
                    <div>
                      <div class="font-bold text-slate-900 dark:text-slate-100">
                        {{ person.name || person.user?.name || 'Unknown User' }}
                      </div>
                      <div class="text-sm font-light text-slate-500 dark:text-slate-400">
                        {{ person.email || person.user?.email || 'No email' }}
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-8 py-6">
                  <span
                    ...
                    :class="
                      person.registration_status === 'confirmed'
                        ...
                        : person.registration_status === 'pending'
                ...
                <td class="px-8 py-6 text-right">
                  <button class="p-2 text-slate-300 transition-colors hover:text-slate-600 dark:text-slate-600 dark:hover:text-slate-300">
